...
class Sabr:
    ...
    def __init__(self,
        ...
        self.gamma[0,0,1] = -b * np.sqrt(1.-rho ** 2)
        self.gamma[0,1,0] = self.gamma[0,0,1]
        self.gamma[1,1,0] = rho / np.sqrt(1. - rho ** 2)
    ...
    def lifted_v(self, 
        ...
        def vec1(x: np.ndarray):
            ...
            """
            v0 = x[2] * self.a * x[0] * x[1]
            v1 = x[2] * self.b * self.rho * x[1] + x[4] * self.b * np.sqrt(1. - self.rho ** 2) * x[1]
            ...
            vec = tf.convert_to_tensor([v0, v1, n0, n1, n2, n3], np.float32) * bm[0]
            return  vec
        ...
        def vec2(x: np.ndarray):
            ...
            x[0]=x1, x[1]=x2, x[2]=e11, x[3]=e12,x[4]=e21,x[5]=e22
            """
            v0 = x[3] * self.a * x[0] * x[1]
            v1 = x[3] * self.b * self.rho * x[1] + x[5] * self.b * np.sqrt(1. - self.rho ** 2) * x[1]
            ...
            vec = tf.convert_to_tensor([v0, v1, n0, n1, n2, n3], np.float32) * bm[1]
            return  vec
        return [vec1, vec2]

    def get_v0(self):
        def vec0(x: np.ndarray):
            v0 = -0.5 * (self.a ** 2 * self.beta * x[1]**2 * np.power(x[0], 2. * self.beta -1.) + self.a * self.b * self.rho * x[1] * np.power(x[0], self.beta))
            v1 = -0.5 * self.b ** 2 * x[1]
